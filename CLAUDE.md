...
**`viewer/` is a consumer, not part of the parser.** A React SPA over `pyraml
tree` output, outside the Python gate and outside `pyraml/`. Its `src/tree.d.ts`
is generated by `python -m pyraml.views.bindings` — never edit it, and a stale
copy fails `tests/unit/test_bindings.py`. Its own checks are `npm run check` in
that directory. It is here because a format with no consumer is a format whose
gaps nothing measures; three fixes so far came from building against it.
