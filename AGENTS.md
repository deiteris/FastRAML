...
- **`contrib/`** — six separate `uv` projects, each with its own lock and gate:
  ...
  `raml-codegen` (the tree → source, with a `python` target) and
  ...
  **A consumer may not hold a rule the language states, and may not invent one
  ...
  `raml-codegen` is the one that reads the *tree* rather than the model, and its
  `raml_codegen/tree.py` is generated by the Python backend under
  `fastraml/views/bindings/` — never edit it either (`docs/16` § 11.11a).
- **`fixtures/`** — one worked document exercising every construct the model
  ...
  the `fastmcp-raml`, `raml-mock` and `raml-codegen` suites — the last of which
  keeps a generated client as a golden record. `shared/` sits outside `sample/`
  so the includes ascend and the workspace root is under test.
