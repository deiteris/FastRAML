...
class TestTheOpenApiSaysWhatTheRamlSaid:
    ...
    async def test_a_facet_reaches_the_schema(self, client):
        # `Book-Input` rather than `Book`: a model with an alias has a different
        # validation schema from its serialisation schema, so FastAPI names the
        # two apart. See `TestAnAliasedPropertyIsAcceptedUnderBothNames`.
        found = await self.schema(client)
...
@pytest.mark.anyio
class TestAnAliasedPropertyIsAcceptedUnderBothNames:
    """The one leniency the generated server has that the document does not.

    `createdAt` is what the document says and `created_at` is what the attribute
    is called, and `populate_by_name=True` makes both work on the way in. The
    alternative is a generated model that cannot be constructed from Python
    without writing the wire names, which is most of what a model is for when
    the thing being written is the server.

    Recorded here rather than left to be discovered: it is a request the
    document does not describe, and the README names it.
    """

    async def test_the_documents_own_name_works(self, client):
        async with client as call:
            assert (await call.post('/books', json=BOOK, headers=BEARER)).status_code == 201

    async def test_the_python_name_works_too(self, client):
        renamed = {key: value for key, value in BOOK.items() if key != 'createdAt'}
        renamed['created_at'] = '2024-01-01T00:00:00'
        async with client as call:
            assert (await call.post('/books', json=renamed, headers=BEARER)).status_code == 201

    async def test_only_the_documents_name_goes_back_out(self, client):
        # Whatever came in, what goes out is what the document describes.
        async with client as call:
            body = (await call.get('/books/9780441013593', headers=BEARER)).json()
        assert 'createdAt' in body
        assert 'created_at' not in body
