...
def _one_ref(node: Node, location: str, *, what: str) -> DirectiveRef:
    ...
    if node.kind is NodeKind.SCALAR:
        if is_null(node):
            ...
            return DirectiveRef(name='', key_pos=node.position, value_pos=node.full_position, is_null_scheme=True)
        return DirectiveRef(name=node.value, key_pos=node.position, value_pos=node.full_position)
    ...
    return DirectiveRef(name=key.value, params=params, key_pos=key.position, value_pos=value.full_position)


def _ref_list(node: Node, location: str, *, what: str) -> list[DirectiveRef]:
    """A sequence of references, or a single one written without the sequence."""
    if is_null(node):
        return []
    if node.kind is NodeKind.SEQUENCE:
        return [_one_ref(item, location, what=what) for item in node.content]
    return [_one_ref(node, location, what=what)]


def decode_type_ref(node: Node, location: str) -> DirectiveRef:
    ...
    ref = _one_ref(node, location, what='resource type')
    if ref.is_null_scheme:
        raise node_error('resource type must not be null', location, node)
    return ref


def decode_trait_refs(node: Node, location: str) -> list[DirectiveRef]:
    """`is:` — a sequence of trait references."""
    refs = _ref_list(node, location, what='trait')
    for ref in refs:
        if ref.is_null_scheme:
...
def decode_secured_by(node: Node, location: str) -> list[DirectiveRef]:
    ...
    """
    return _ref_list(node, location, what='security scheme')
