...
* **Analysis and linting**: run custom SPARQL or one of 9 named graph queries. `lint` checks the effective model against 73 built-in rules, with opt-in security (OWASP and OAuth), HTTP semantics (RFC 9110), problem details (RFC 9457) and style rulesets, per-rule explanations and plugins ([Linting](#linting)).
...
sees what a client of the API sees. The 73 built-in rules fall into five
...
* **`security`** (28 rules, opt-in): derived from the
  ...
  string, passwords in `baseUri`, HTTPS-only transport and OAuth endpoints), unrestricted resource
...
* **`http`** (18 rules, opt-in): what
  ...
  200's validators; responses the declared request can never produce; resource
  paths with characters a URI path forbids or with `.`/`..` segments
  ([RFC 3986](https://www.rfc-editor.org/rfc/rfc3986)); a `charset` on JSON
  ([RFC 8259](https://www.rfc-editor.org/rfc/rfc8259)); and a media type
  declared twice in different case.
