...
| [`raml-codegen`](raml-codegen/) | RAML → code | Generates source from `fastraml tree` output. One target so far: `python`, a typed `httpx` client. |

`raml-document` is the part the other two share, so that two integrations cannot
disagree about what a RAML document is.

`raml-codegen` is the odd one out, deliberately: it reads the *tree* — the
published JSON wire format — rather than `fastraml.Raml`. Its
`raml_codegen/tree.py` is generated by `python -m fastraml.views.bindings python`
and is checked by the root gate; never edit it by hand.
