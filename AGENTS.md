...
- **`contrib/`** — six separate `uv` projects, each with its own lock and gate:
  ...
  `raml-codegen` (a tree document → source, with a `python` target that writes
  an `httpx` client and a `fastapi` target that writes the server interface to
  implement, and depending on no parser) and
  ...
  **A consumer may not hold a rule the language states, and may not invent one
  ...
  `targets/shared/` holds the reading of the tree, so a target is six spelling
  hooks and a set of templates; a change that alters the `python` golden while
  claiming to touch only the `fastapi` one is the refactor having gone wrong.
