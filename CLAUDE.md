...
Nothing under `pyraml/` may import one, and none of them may hold a rule the
...
- **`viewer/`** — a React SPA over `pyraml tree` output, outside the Python
  gate. Its `src/tree.d.ts` is generated by `python -m pyraml.views.bindings` —
