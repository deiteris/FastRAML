...
- **`contrib/`** — six separate `uv` projects, each with its own lock and gate:
  ...
  `raml-codegen` (a tree document → source, with a `python` target, and
  depending on no parser) and
  ...
  **A consumer may not hold a rule the language states, and may not invent one
  ...
  Python backend under `fastraml/views/bindings/` — never edit it either.
