"""Template variables: docs/08-templates-and-endpoints.md § 5.
...
class TestPluralizationParity:
    ...
    go-raml is to share its dictionary: `go-pluralize` and
    ...
    `data/pluralize_parity.tsv` was generated by running go-raml's own transform
    over go-pluralize's whole irregular and uncountable tables. See docs/08 § 5.
    """
...
class TestVariablesIndex:
    ...
    def test_the_key_is_the_scalar_node_itself(self):
        # `Node` is identity-hashable by design (docs/03 § 2), so the node
...
class TestCollectRequiredVariables:
    ...
    def test_the_index_survives_a_subtree_being_filtered_out(self):
        # docs/08 § 3.1: optional methods are removed from the tree
