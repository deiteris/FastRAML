...
The argument is the TypeScript backend's, and it does not weaken inside Python:
`build_tree` returns `Json`, so a Python consumer of the wire format reads
`dict[str, object]` and finds a missing key at run time. The boundary a type
checker fails to span is the JSON, not the language.

This backend contains no Python source analysis. `schema.py` owns that, and what
is left here is the same two jobs the TypeScript backend has: map the neutral
schema's annotations and wire forms into a type system, and supply the value type
of each *structural* key, which no source analysis can infer from
`out['operations']`. The key sets still come from the schema, so **a key added to
the projection and not declared here fails generation by name.**

Four things TypeScript spells for free and Python does not:

* **An optional key** is `NotRequired[T]`, per key. `total=False` would make every
  key optional and discard the required/optional split `schema.py` derives.
* **`$ref` is not an identifier**, so `Ref` needs `TypedDict`'s functional form.
* **A template literal type has no Python spelling.** `SecuritySchemeType` closes
  over the six the spec names and widens to `str` for the `x-` case, rather than
  pretending the whole vocabulary is closed.
* **A `TypedDict` subclass may not re-declare a key**, so `type` cannot be
  narrowed from `ShapeType` to `Literal['object']` by inheritance. `ShapeBase`
  omits it and each variant declares its own — which is what the TypeScript
  backend already does, so the discriminated union costs nothing extra.

`python -m fastraml.views.bindings python -o FILE` selects this backend and writes
a destination chosen by its caller. `tests/unit/test_bindings.py` asserts that
...
GENERATED by `python -m fastraml.views.bindings python` -- do not edit. The key
sets are read from `fastraml/views/tree.py` and the facets from the kind classes
in `fastraml/types/`, so a facet added to a kind arrives here without this file
...
#: The small fixed records nested inside structural fields. `ShapeNode` is a
#: string alias, so `ShapeNode | None` has to be quoted whole -- the same rule
#: `_Behind` applies to the generated fields.
_RECORDS: Final = (
...
def python() -> str:
    ...
    # quoted whole.
    behind = _Behind(set(_declared_in(_PREAMBLE)) - set(_STRING_ALIASES.findall(_PREAMBLE)))
    blocks = [_PREAMBLE.rstrip('\n')]

    blocks.append(_vocabularies(schema))
    behind.add(vocabulary.name for vocabulary in schema.vocabularies)

    for block in (*_shape(schema, behind), *_typed_dicts(schema.projector, behind), *_RECORDS):
        blocks.append(block)
...
def _shape(schema: ContractSchema, behind: _Behind) -> list[str]:
    ...
    return [head, *variants, _union_alias('Shape', tuple(by_model))]
