...
- **`viewer/`** — a React SPA over `fastraml tree` output, outside the Python
  ...
  typescript -o viewer/src/tree.d.ts` —
...
- **`contrib/`** — six separate `uv` projects, each with its own lock and gate:
  ...
  **A consumer may not hold a rule the language states, and may not invent one
  ...
  parser (`docs/17` § 5.2); its `raml_codegen/tree.py` is generated by the
  Python backend under `fastraml/views/bindings/` — never edit it either. Its
