...
A consumer of `pyraml tree` has to know what the JSON holds. Written by hand
...
`python -m pyraml.views.bindings` writes the file. `tests/unit/test_bindings.py`
...
def _kinds() -> dict[str, list[_Facet]]:
    ...
    for path in sorted((_ROOT / 'pyraml' / 'types').glob('*.py')):
