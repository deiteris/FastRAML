"""A pyRAML shape -> JSON Schema draft-07.
...
- **The entry point must be unwrapped.** go-raml refuses otherwise
  (`entrypoint shape must be unwrapped`), and for the same reason pyRAML's
...
- **Numbers never pass through `float` on the way in.** A pyRAML facet holds a
