...
- Code generation, HTTP middleware, an LSP server, and AMF conversion. The model
  is designed so these can be built on top.

  **JSON Schema and OpenAPI are the exceptions, and both are in.**
  `views/jsonschema.py` converts
