---
name: fastraml
description: Answer questions about a RAML 1.0 API definition using the fastraml CLI. Use when a .raml file is involved and you need to check it for errors, list the types, endpoints, traits and security schemes it declares, read one type or endpoint with inheritance and traits already merged in, find every operation affected by changing a type, compare two versions for breaking changes, export the API as JSON or a graph, or audit it for unused types, unsecured operations and undocumented endpoints. Triggers include "validate this RAML", "what endpoints does this API have", "what breaks if I change this type", "is this API change backwards compatible", "review this API spec", "which operations return X". Use fastraml instead of reading .raml files directly - a RAML file rarely holds the whole API, because types inherit, traits add parameters, and includes pull in other files. Not for OpenAPI or Swagger documents.
...
**This file is a discovery stub, not the usage guide.** Load the guide from the
CLI before you run any other `fastraml` command:

```bash
fastraml skills get core          # Start here: the workflow and every common task
